    ...
    <body>
        ...
        def amount(text):
            ...
            if '-' in text :
               ...
               LongText=float(text)
               return ('{0:,.2f}'.format(LongText)) + ' D'
               
        def th_separator(text):
               LongText=float(text)
               return ('{0:,.2f}'.format(LongText))
        %>
                   ...
                   <%
                    ...
                    <div class="act_as_row lines ${level_class} ${"%s_account_type" % (current_account.type,)}">
                        ...
                        %if comparison_mode == 'no_comparison':
                            %if initial_balance_mode:
                                ## opening balance
                                <div class="act_as_cell amount">${current_account.init_balance}</div>
                                ##<div class="act_as_cell amount">${formatLang(current_account.init_balance) | amount}</div>
                                
                            %endif
                            ## debit
                            <div class="act_as_cell amount">${formatLang(current_account.debit) | th_separator}</div>
                            ## credit
                            <div class="act_as_cell amount">${formatLang(current_account.credit) | th_separator}</div>
                        %endif
                        ## balance
    ...
    <div class="act_as_table list_table" style="margin-top: 5px;">
            <div class="act_as_thead">
                <div class="act_as_row labels">
                    ...
                    %if comparison_mode == 'no_comparison':
                    	<div class="act_as_cell amount" style="width: 30px;">${formatLang(total_init_balance) | amount}</div>
                    	<div class="act_as_cell amount" style="width: 30px;">${formatLang(total_debit) | th_separator}</div>
	                    <div class="act_as_cell amount" style="width: 30px;">${formatLang(total_credit) | th_separator}</div>
    	                <div class="act_as_cell amount" style="width: 30px;">${formatLang(total_final_balance) | amount}</div>
