...
class HeaderFooterTextWebKitParser(webkit_report.WebKitParser):
    ...
    def create_single_pdf(self, cursor, uid, ids, data, report_xml, context=None):
        ...
        if report_xml.report_file:
            if os.path.sep in report_xml.report_file :
                path = addons.get_module_resource(*report_xml.report_file.split(os.path.sep))
            else :
                report_file = report_xml.report_file.replace('\\',os.path.sep)
                path = addons.get_module_resource(*report_file.split(os.path.sep))
